...
import { MousePointerClick, RefreshCw, Timer } from "lucide-react"; // Changed HandClick to MousePointerClick
...
const ClickerChallengeGame = () => {
  ...
  return (
    <Card className="w-full max-w-md mx-auto bg-white dark:bg-gray-700 shadow-xl rounded-lg p-6 text-center">
      <CardHeader>
        <MousePointerClick className="h-16 w-16 text-teal-600 dark:text-teal-400 mx-auto mb-4" /> {/* Changed icon */}
      ...
      <CardContent className="space-y-6">
        <div className="flex justify-around items-center text-2xl font-semibold text-gray-800 dark:text-gray-200">
          <p className="flex items-center gap-2">
            <MousePointerClick className="h-6 w-6 text-blue-600 dark:text-blue-400" /> Clicks: {clicks} {/* Changed icon */}
          </p>
          <p className="flex items-center gap-2">
      ...
      <CardFooter className="flex justify-center gap-4 mt-6">
        {!isRunning && !gameOver && (
          <Button onClick={startGame} className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600">
            <MousePointerClick className="h-4 w-4 mr-2" /> Start Challenge {/* Changed icon */}
          </Button>
        )}
        {(isRunning || gameOver) && (
